 ...
 * Required shape: `new Hono().use("*", requireSession).get(...)`; admin variants also add `.use("*", requirePermission(PERMISSIONS.X))`.
 * `config` paths are intentionally excluded — deliberately unauthenticated, confirmed by AI review.
 */
