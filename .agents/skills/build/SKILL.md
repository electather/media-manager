...
2. read the skill for clean-code skill
3. before writing code, check memory for patterns & context via token-savior. Avoid re-deriving what's already known. Reuse symbol clusters, call chains, type designs from memory index.
4. Edit code per plan.
5. Run verification command.
6. **Pass** → flip `~` → `x`. Next task.
7. **Fail** → invoke backprop skill. Do NOT retry blindly.
