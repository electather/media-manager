...
export function MediaDetailModalContent({
  ...
  }, [item.id, loading, enableScrollAnimations]);

  useEffect(() => {
    if (loading || !enableScrollAnimations) return;
    const measure = () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, item.id, enableScrollAnimations]);

  useEffect(() => {
    if (!loading && enableScrollAnimations) requestAnimationFrame(applyScrollStyles);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, enableScrollAnimations]);
    ...
    <>
      ...
      {!loading && enableScrollAnimations && (
        <div
          ref={floatingLogoRef}
          aria-hidden="true"
          className="pointer-events-none absolute z-10 flex h-11 items-center"
          style={{
            top: logoBoxRef.current.fromTop,
      ...
      <div
        ref={scrollRef}
        className={cn(
          "relative z-0 size-full overflow-y-auto",
          enableScrollAnimations && "scroll-smooth",
        )}
      >
        {loading ? (
          <div className="relative">
            {enableScrollAnimations && <div className="h-[40vh]" aria-hidden="true" />}
            ...
            <div className="relative bg-background">
              <div className="relative px-7 pt-5 pb-7">
                <div
                  ref={logoSlotRef}
                  aria-hidden="true"
                  className="mb-3.5 flex min-h-[44px] items-center invisible"
                >
                  <span
                ...
                {(item.rating || item.audienceScore != null || item.criticScore != null) && (
