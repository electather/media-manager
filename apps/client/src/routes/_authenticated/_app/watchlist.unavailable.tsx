...
export const Route = createFileRoute("/_authenticated/_app/watchlist/unavailable")({
  validateSearch: searchSchema,
  // Only `sort` shapes the first page; `peek` (the modal) must not re-run the loader.
  loaderDeps: ({ search }) => ({ sort: search.sort }),
  loader: ({ context: { queryClient }, deps: { sort } }) =>
    prefetchMediaRows(queryClient, watchlistItemsSource({ sort, bucket: "unavailable" })),
  errorComponent: WatchlistRouteError,
  component: () => <WatchlistFlatPage bucket="unavailable" />,
});
