...
export class AuthService {
   ...
   * Guards against assigning admin-capable roles: setting a new account's password then logging in
   * achieves immediate privilege escalation. `systemSlug` passed separately — system Admin has no rows in `role_permissions`.
   */
  ...
  async requireSession(c: Context, next: Next): Promise<void> {
    ...
    // expires, because the signed cookie is self-contained and deleting the DB row cannot
    // invalidate it (#926). Sessions live server-side, so the cache is only an optimisation
    // here; bypassing it costs one lookup but closes the revocation gap on every route.
