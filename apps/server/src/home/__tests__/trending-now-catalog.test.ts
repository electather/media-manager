...
function makeMediaServiceStub() {
  return {
    getTrendingFeed: vi.fn(async () => ({
      items: [
        {
          id: "movie:1",
          type: "movie",
          title: "Live Trending 1",
          ids: { tmdb_id: "1" },
        },
      ],
      partial: false,
...
describe("trendingNow fetcher catalog hydration", () => {
  it("hydrates from the snapshot and surfaces canonical artwork", async () => {
    const media = makeMediaServiceStub();
    const key: MetadataKey = { tmdbId: "550", type: "movie" };
    const ctx = makeCtx(media, {
      snapshot: [key],
      rows: { "movie:550": buildCanonical(key, "Cached Trending") },
    });

    const result = await trendingNowFetcher.fetch(ctx, { cursor: null, limit: 20 });
    expect(result.items).toHaveLength(1);
    expect(result.items[0]?.title).toBe("Cached Trending");
    expect(result.items[0]?.poster).toBe("https://image.tmdb.org/t/p/w500/poster.jpg");
    expect(result.items[0]?.backdrop).toBe("https://image.tmdb.org/t/p/w1280/backdrop.jpg");
    expect(media.getTrendingFeed).not.toHaveBeenCalled();
  });

  it("falls back to the live plugin when the snapshot is missing", async () => {
    const media = makeMediaServiceStub();
    const ctx = makeCtx(media, { snapshot: null, rows: {} });

    const result = await trendingNowFetcher.fetch(ctx, { cursor: null, limit: 20 });
    expect(result.items).toHaveLength(1);
    expect(result.items[0]?.title).toBe("Live Trending 1");
    expect(media.getTrendingFeed).toHaveBeenCalledOnce();
  });

  it("falls back to the live plugin when the snapshot is empty", async () => {
    const media = makeMediaServiceStub();
    const ctx = makeCtx(media, { snapshot: [], rows: {} });
