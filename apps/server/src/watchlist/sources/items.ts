...
function compareAlpha(aMeta?: CanonicalMetadata, bMeta?: CanonicalMetadata): number {
  ...
  // produce different orderings for accented or non-ASCII titles. The "base"
  // sensitivity makes the compare case- and accent-insensitive, which replaces
  // the previous host-default toLocaleLowerCase()/normalize() pre-pass — that
  // step was itself locale-dependent (e.g. Turkish dotless-i) and is redundant
  // here. No numeric collation: ordering stays purely lexical, matching the
  // pre-fix behaviour.
