...
| T34 | x      | Move shared — `components/{ui,error-boundary,logo,not-found,user-avatar,log-viewer,json-viewer,pickers,cron-schedule}` → `shared/`. `hooks/use-now.ts` → `shared/hooks/`. `lib/*` → `shared/lib/`. Add `client-shared-*` fallow zones, drop legacy `client-{components,hooks,lib}`. **Mechanical: `git mv` + import path update only, ⊥ rewrites (C13).** | C12,C13,V51,V55,V58,V60   |
