...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const key = typeof rawLabelKey === "function" ? "value" : `${rawLabelKey}`;
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            const rawNameKey = nameKey ?? item.name ?? item.dataKey ?? "value";
            const key = typeof rawNameKey === "function" ? "value" : `${rawNameKey}`;
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
    ...
    >
      {payload
        ...
        .map((item, index) => {
          ...
          const key = typeof rawKey === "function" ? "value" : `${rawKey}`;
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
