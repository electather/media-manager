...
import type { MetadataProvider, ActivityProvider, DownloadProvider, DiscoverFilters } from '../integrations/types'
...
export class MediaService {
  constructor(
    ...
    private readonly downloads: DownloadProvider,
    private readonly cache: CacheProvider,
    private readonly preferences: PreferenceEngine,
  ) {}

  async search(_query: string, _mediaType?: 'movie' | 'tv', _limit?: number): Promise<SearchResult[]> {
  ...
  async requestDownload(_id: string, _seasons?: string): Promise<{ success: boolean; message: string }> {
