...
| 3   | discover_snapshots + `discover_snapshot` job; `newReleases` row hydration (live fallback). `trendingNow` and `upcomingForYou` hydration deferred — their existing fetchers source from `recommendations@v1.getTrending` and per-user `calendar@v1.getUpcoming`, which produce different semantic feeds than the discover-sourced snapshot. The snapshot job still writes their tuples; a follow-up reconciles the row sources before they consume the catalog. |
