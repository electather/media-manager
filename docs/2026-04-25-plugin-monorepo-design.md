...
Refactor split monorepo: each integration own publishable workspace package, new `@nama/plugin-sdk` as single dep every plugin author imports, top of tree reorganised into conventional `apps/` (runnable) + `packages/` (publishable libs) split. Each plugin package build with `vp pack` (Vite+'s tsdown wrapper) into single-file artifact (`dist/plugin.js`) for distribution + forward-compat with future third-party install. Built-ins keep loading via workspace TS imports in dev + prod.

Versioning stay on Changesets, independent versions per package. Apps + publishable packages get GitHub Releases; `@nama/shared` internal-only.

## Goals

- Each integration (Trakt, TMDB, Plex, Jellyfin, Seerr, TVDB, Fanart) own workspace package, independent versioning under Changesets.
- Move runnable apps under `apps/`, reusable libs under `packages/` so monorepo shape reflect artifact purpose.
- `@nama/plugin-sdk` = single dep every plugin author import — types, helpers, error class, capability schemas, validator, testing kit. Versioned with `manifest.sdkVersion` as install-time gate.
...
1. **PR [#107](https://github.com/electather/nama/pull/107)** — flip `client` and `server` to `private: false` so Changesets tag them and `changesets/action` produce GitHub Releases. Plan assume convention (`private: true` ⇒ no Release; `private: false` ⇒ tagged + Released).
2. **Issue [#106](https://github.com/electather/nama/issues/106)** — relocate `HOST_ERROR_CODES` and `HostErrorCode` from `apps/server/src/errors/codes.ts` to `@nama/shared/errors`. SDK re-exports type for plugin authors. Self-contained PR.
...
```
apps/
  client/                     @nama/client       (private: false, never npm-published; Cloudflare Assets bundle)
  server/                     @nama/server       (private: false, never npm-published; Docker image release artifact)
packages/
  shared/                     @nama/shared       (private: true; isomorphic types/schemas, no runtime deps besides zod)
  plugin-sdk/                 @nama/plugin-sdk   (private: false; plugin author API + testing kit)
  plugins/
    trakt/                    @nama/plugin-trakt
    tmdb/                     @nama/plugin-tmdb
    plex/                     @nama/plugin-plex
    jellyfin/                 @nama/plugin-jellyfin
    seerr/                    @nama/plugin-seerr
    tvdb/                     @nama/plugin-tvdb
    fanart/                   @nama/plugin-fanart
```
...
1. **Plugins depend only on `@nama/plugin-sdk`.** Never on `@nama/shared`, never on `@nama/server`, never on each other. Anything plugin needs must re-export from SDK.
2. **`@nama/shared` stay free of plugin concerns.** Plugin-specific stuff shared between client + server stay where it is (`@nama/shared/plugins` for manifest schema, library types). SDK _re-exports_ for plugin authors but does not move. Shared keep existing rule: isomorphic data shapes, zod-only runtime dep.

## `@nama/plugin-sdk` package contents

**Hard rule: every symbol = exactly one canonical home. SDK either _owns_ symbol (moved from elsewhere) or _re-exports_ from `@nama/shared`. Nothing duplicated.**

`@nama/shared` internal to monorepo (Bun workspace dep, never published). Third-party plugin authors only depend on `@nama/plugin-sdk`. Shared types plugins need (manifest schema, error codes) reach plugin authors via SDK re-export, while shared stay canonical source for first-party consumers.
...
#### RE-EXPORTED (canonical stays in `@nama/shared`)
...
| `JSONSchema` type, `McpToolAnnotations`, library types under `@nama/shared/plugins/library` | `packages/shared/src/plugins/library.ts` + `types.ts`          | Re-export from SDK.                                                                                                                                        |
| `HostErrorCode` union                                                                          | `packages/shared/src/errors/codes.ts` (after issue #106 lands) | Re-export from SDK.                                                                                                                                        |
| `ArtworkBundle`, `ArtworkVariant`, `ArtworkIdMap` types                                        | `packages/shared/src/artwork/`                                 | Re-export from SDK so plugin authors implementing `artwork@v1` get the bundle / variant / id-map types via `@nama/plugin-sdk` w/o reaching into shared. |
...
### `@nama/plugin-sdk/testing` subpath
...
packages/plugins/trakt/
  ...
  __tests__/
    contract.test.ts       # uses @nama/plugin-sdk/testing
...
`bytes: 'builtin:${id}@${version}'` stay synthetic identifier. With independent package versions, manifest version IS package version, so bump to `@nama/plugin-trakt@1.4.0` flow naturally to synthetic string and trigger `bootstrapBuiltins` to refresh row.
...
| ----------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Per-plugin contract tests     | `packages/plugins/<id>/__tests__/contract.test.ts`             | Drive every declared capability method end-to-end with stubbed `PluginContext`; assert request URLs and parse results against capability Zod output schema. Imports `validatePluginModule`, `makeTestContext`, fetch helpers from `@nama/plugin-sdk/testing`. Today's pattern, relocated. |
...
| ----------------------------- | --------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `@nama/server`             | `false`   | Yes                      | Docker image pushed to GHCR (`ghcr.io/electather/nama:<version>`), linked from Release notes; plus source archive |
| `@nama/client`             | `false`   | Yes                      | Built static bundle attached as Release asset (so self-hosters can serve from any static host without rebuilding)          |
| `@nama/shared`             | `true`    | No                       | —                                                                                                                          |
| `@nama/plugin-sdk`         | `false`   | Yes                      | `dist/` tarball                                                                                                            |
| `@nama/plugin-<id>` (each) | `false`   | Yes                      | `dist/plugin.js` + `dist/plugin.d.ts` + manifest snapshot                                                                  |
...
- For `@nama/server`: build Docker image + **push to GitHub Container Registry (`ghcr.io/electather/nama:<version>`)**, then add line to Release notes linking GHCR URL. Docker images not attached as Release assets (oversized for asset mechanism, GHCR = conventional pull surface for self-hosters). Docker build job itself new — existing `release.yml` does not build images today — so this commit also introduce build/push step.
- For `@nama/client`: attach built static bundle (`dist/`) as Release asset so self-hosters can serve from any static host without rebuilding.

## Boundary enforcement

`scripts/check-plugin-deps.ts` (or equivalent ESLint rule) fail CI if any file under `packages/plugins/` import from `@nama/shared` or `@nama/server`. Plugins must reach those through `@nama/plugin-sdk` re-exports. Wired into `vp check`. Single rule keeps boundary honest as codebase grows.
...
2. **Create `@nama/plugin-sdk` skeleton with relocated types and helpers.** New package at `packages/plugin-sdk/`. Move plugin-author types, `definePlugin`, `validatePluginModule`, `isSdkCompatible` + `SDK_VERSION`, `pluginError`, `handleHttpStatus`, `resolveCredential`. Update server imports. Plugins still live at `apps/server/src/plugins/builtin/` and import from SDK now.
3. **Move capability schemas to the SDK.** Split `apps/server/src/plugin-runtime/capabilities.ts` (single 870+ line file holding every `defineCapability(...)` export plus `CAPABILITY_CATALOG`, `getCapability`, `listCapabilities`) into `packages/plugin-sdk/src/capabilities/<capability>.ts` with barrel `index.ts` rebuilds catalog + re-exports lookup helpers. **Create** new `apps/server/src/plugin-runtime/register-capabilities.ts` imports each capability def from SDK + calls `registerCapability(...)` to populate runtime dispatch registry. Update plugin contract tests to import schemas + helpers from `@nama/plugin-sdk` instead of relative server paths.
4. **Create `@nama/plugin-sdk/testing` subpath.** Extract `makeTestContext`, `jsonRes`, `statusRes`, `paginatedPage` from duplicated patterns across `apps/server/src/plugins/builtin/*/__tests__/`. Plugin tests adopt shared fixtures.
5. **Extract `@nama/plugin-tvdb`.** First plugin extraction — smallest blast radius first. Apply per-plugin template (see below), ship as one commit.
6. **Extract `@nama/plugin-tmdb`.** Same template.
7. **Extract `@nama/plugin-seerr`.** Same template.
8. **Extract `@nama/plugin-trakt`.** Same template.
9. **Extract `@nama/plugin-plex`.** Same template.
10. **Extract `@nama/plugin-jellyfin`.** Same template — biggest, intentionally last so earlier extractions de-risk pattern.
11. **Delete the husk.** Remove now-empty `apps/server/src/plugins/builtin/` and `apps/server/src/plugins/utils/` directories.
12. **Add the boundary lint check.** `scripts/check-plugin-deps.ts` failing on plugin-package imports from `@nama/shared` or `@nama/server`. Wired into `vp check`.
...
14. **Release workflow updates.** Adjust `.github/workflows/release.yml` to (a) build + attach `dist/plugin.js` + `dist/plugin.d.ts` as Release assets for each tagged plugin package, (b) build + attach SDK `dist/` tarball to its Release, (c) build + push server Docker image to GHCR + link pull URL from `@nama/server` Release notes, (d) build + attach client static bundle as asset on `@nama/client` Release. Docker build job itself new — `release.yml` does not build images today.

**Per-plugin extraction template (commits 5–10).** For each `<id>` in TVDB, TMDB, Seerr, Trakt, Plex, Jellyfin: create `packages/plugins/<id>/` with `package.json`, `vite.config.ts`, `src/`, `__tests__/`; add `@nama/plugin-<id>` as dep in `apps/server/package.json`; update `registry.ts` to import from new package; delete old `apps/server/src/plugins/builtin/<id>/` directory; write changeset (`@nama/plugin-<id>: minor` initial release + `@nama/server: patch` consumer update).

**Greenfield additions (post-migration).** `@nama/plugin-fanart` is new — ⊥ exists in `apps/server/src/plugins/builtin/` to extract. Lands directly at `packages/plugins/fanart/` per same per-plugin template (minus the deletion step). Tracked in `docs/2026-04-26-plugin-fanart-design.md`. Same shape applies to any future built-in plugin.
...
4. Write `.changeset/<slug>.md` naming `@nama/plugin-trakt`.
...
- PR [#107](https://github.com/electather/nama/pull/107) — release-page convention dependency.
- Issue [#106](https://github.com/electather/nama/issues/106) — `HOST_ERROR_CODES` relocation pre-flight dependency.
