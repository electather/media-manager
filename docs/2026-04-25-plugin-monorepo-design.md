...
This refactor splits the monorepo so that each integration is its own publishable workspace package, introduces a dedicated `@ent-mcp/plugin-sdk` as the single dependency every plugin author imports, and reorganises the top of the tree into the conventional `apps/` (runnable) and `packages/` (publishable libraries) split. Each plugin package builds with `vp pack` (Vite+'s wrapper around tsdown) into a single-file artifact (`dist/plugin.js`) for distribution and forward-compatibility with future third-party install, while built-ins continue to load via workspace TypeScript imports in dev and prod.
...
- Each plugin builds to a single-file artifact via `vp pack` (Vite+'s wrapped tsdown) (`dist/plugin.js`), matching the shape third-party plugins will eventually be loaded as.
...
Unlike plugin packages (which use a `development`/`default`/`types` conditional, see below), the SDK's exports always resolve to TypeScript source. The server runs from source via Bun in dev and prod, and the SDK is never loaded as a pre-built bundle, so a `dist/` condition would add a code path nothing reads. When the SDK gains an npm distribution, that's the moment to add a `default` condition pointing at a built artifact — not before.
...
Vite+ wraps tsdown via `vp pack`; per the Vite+ migration guide, packaging options live in the `pack` block of `vite.config.ts`, not a standalone `tsdown.config.ts`. There is no direct `tsdown` import or devDependency — `vite-plus` provides the wrapper.
...
The SDK's `vite.config.ts` reads its own `package.json` version and substitutes via the `pack.define` option (forwarded to tsdown). Single source of truth.
...
5. **Extract `@ent-mcp/plugin-tvdb`.** First plugin extraction — smallest blast radius first. Apply the per-plugin template (see below) and ship as one commit.
...
10. **Extract `@ent-mcp/plugin-jellyfin`.** Same template — the largest, intentionally last so the earlier extractions de-risk the pattern.
...
**Per-plugin extraction template (commits 5–10).** For each `<id>` in TVDB, TMDB, Seerr, Trakt, Plex, Jellyfin: create `packages/plugins/<id>/` with `package.json`, `vite.config.ts`, `src/`, `__tests__/`; add `@ent-mcp/plugin-<id>` as a dependency in `apps/server/package.json`; update `registry.ts` to import from the new package; delete the old `apps/server/src/plugins/builtin/<id>/` directory; write a changeset (`@ent-mcp/plugin-<id>: minor` initial release + `@ent-mcp/server: patch` consumer update).
