...
**Greenfield additions (post-migration).** `@ent-mcp/plugin-fanart` is new — ⊥ exists in `apps/server/src/plugins/builtin/` to extract. Lands directly at `packages/plugins/fanart/` per same per-plugin template (minus the deletion step). Tracked in `docs/2026-04-26-plugin-fanart-design.md`. Same shape applies to any future built-in plugin.
