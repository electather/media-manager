...
Single-writer invariant: `service.syncMembership` is the sole membership writer and runs inside a per-user promise-chain mutex (`syncMutex`, `internal/sync-mutex.ts`, mirrors catalog's `PerUserMutex`). Serializes `library.sync` (cron) vs the eager-seed path per user, so `tombstoneMissing`'s non-atomic read-then-update slow path is provably single-writer — the job-runner overlap guard alone does not cover cron-vs-eager-seed (#911, ledger 16). Process-local; multi-instance would need Postgres advisory locks (out of scope for v1). Defence-in-depth: the slow path additionally wraps its read-then-sweep in a `db.transaction` (libsql `BEGIN IMMEDIATE` in local-file/embedded-replica modes), so the sweep stays atomic even across processes if the single-instance assumption ever breaks.
...
16. **#911 — `tombstoneMissing` slow path was not provably single-writer.** The `>900` keep branch does a non-atomic read-then-update (collect owned ids, then tombstone the absent ones), safe only under one writer per user. The job-runner overlap guard serializes `library.sync` against itself but not against the eager-seed path (`ensureSeeded` → `syncMembership` on first read), so a cron sweep and an eager seed could interleave and let a row inserted-then-absent after step 1 escape tombstoning. Now: `service.syncMembership` (the sole membership writer) runs inside a per-user promise-chain mutex (`syncMutex`, mirrors catalog's `PerUserMutex`), so all writers for a user serialize and the slow path is single-writer by construction. Process-local; multi-instance would need Postgres advisory locks (out of scope for v1). Belt-and-braces (#1029): the slow path also wraps its read-then-sweep in a `db.transaction` — libsql `BEGIN IMMEDIATE` (local-file/embedded-replica modes) holds a write lock from the start — so the read and sweep commit atomically even against a second process, closing the window without depending on the single-instance mutex.
