...
- **Reusing `ConnectionModal`.** Modal currently consumes `availablePlugins` from outer settings page. Settings/notifications must filter `GET /api/notifications/plugins` (≠ general `/api/connections/available`) ⇒ inject filtered list. ⊥ breaking — modal already takes `plugin: PluginSummary | null` prop.
