...
- **`query-keys.ts`** — `mediaKeys` root + factory: `mediaKeys.source(sourceId, params)`, `mediaKeys.sourceAll(sourceId)` — prefix key sweeping all param variants of one source; use for scoped retry/invalidation without evicting other sources (#876), `mediaKeys.title(type, tmdbId)`, `mediaKeys.counts()`, `mediaKeys.moods()`. home/watchlist key factories **derive** from `mediaKeys` (⊥ independent roots → one-shot invalidation, #505/#514).
