...
- `SourceContext` = unify `RowContext` ∪ `WatchlistContext`. Research: `WatchlistContext` already documented "structurally compatible with home row context"; `asWatchlistContext` already bridges `log`/`logger`. Pin: `{ userId, mediaService, catalog, deadlineMs?, statusBatch, logger }`. Drop the `log` alias (pre-stable break). Two optional consumer-injected callbacks were added in Phase 1 to break cycles (`getArtwork`, `toCanonicalRow`, §C/enrich), and an optional `seedTitle?` in Phase 5 (the home seed source stashes it for the match-reason chip — completing the `RowContext ∪` unification; media never reads it). home passes its `RowContext` straight through as a `SourceContext` (structural superset).
...
- **Enrich is pluggable.** `enrich(rows, batch)` is the *default* (watchlist) projection over persisted `ActiveRow`s. `listRows` takes an optional `enrichRows(rows) → {items, partial}` strategy (Phase 5): when supplied, the default `batchLoad` + `enrich` is skipped entirely. **home** supplies it because its feed rows are not `ActiveRow`s and its enrichment owns the row-aware match-reason chip (`enrichHomeItems` → `enrichCompactItems`, which media's `enrich` lacks). The home strategy = its catalog projection (`fromCanonicalMetadata` / `fromContinueWatchingEntry`) + `enrichCompactItems`. The pipeline still owns classify/filter/sort/paginate.
- `classify` count loop (the 3× dup) is now this single stage; count-mode (§G) reuses it without enrich/paginate.
- `paginate`: keyset mode hops the raw query (overshoot helper, preserves #500 empty-streak `cursor:null` + #501 single-pass sparse bucket+sort + RISK-005 offset ceiling — §E). offset mode slices the in-memory sorted set. A home **offset** row's source returns the full set and the projection runs over all of it before the slice (parity-neutral with a fully-projected fixture; a projection drop shifts the window vs the pre-refactor slice-then-project order — acceptable, within the bounded feed sizes RISK-005 covers). A home **bounded** row's projection slices to a single page so `paginate` mints `cursor:null`.
...
- **home** — `composeLayoutLive`: per `ROW_ORDER`, run `eligibility(ctx)` (consumer-side, §B), then run the row through `listRows` (via `provider.load` → `loadRowPage`), build stub (include iff `items>0 || partial`). hero / match-reason / layout-cache / detail / season composition unchanged. home's 12 rows reimplemented as `MediaSource` (`fetchRawSet` only — sort/slice/cursor deleted from each, → pipeline); the `RowProvider` shape becomes `{ rowId, kind, titleKey, eyebrowKey?, cursorMode, requiresInitialCursor?, eligibility, initialCursor, load }`, with `load` wiring the source + raw-row projection + the home enrich strategy (§C) into `listRows`. `_shared.ts` helpers (`resolveSimilarCandidates`, `loadCanonicalItems`, `probeMediaEntry`) stay home-side (catalog feed plumbing); the home↔media pipeline bridge (`loadRowPage`, `makePipelineRow`, `makeBoundedRow`) lives in `home/internal/pipeline.ts` so the env-free `_shared` helpers + source unit tests never drag the `media → db → env` graph.
  - **Cursor (V.CU1).** `composeRow` decodes the incoming cursor against the row's `cursorMode` via the unified codec (§E): an absent cursor is the first page (unless `requiresInitialCursor`, → 400 `cursor_required`); a bad/foreign/mode-mismatched cursor (`decode → null`) → `HttpError 400`. Seed rows (`becauseYouWatched`/`similarTo`) are **keyset** with the seed `{ seedId, seedType, offset }` riding inside the cursor's `k` (§E); `becauseYouWatched` mints the first seed cursor in `initialCursor`, `similarTo` gets it from the client. The seed source resolves the seed title during `fetchRawSet` and stashes it on `ctx.seedTitle` for the match-reason chip. The old home base64-JSON cursor codec (`home/internal/cursor.ts`) is deleted here (it would otherwise be dead — `unused-files` is an error gate); the `similarTo` client cursor format follows in the client doc (pre-stable, §I).
  - **`compositeId` → `keyToId`** (§F): home's card id now uses the shared `keyToId`.
