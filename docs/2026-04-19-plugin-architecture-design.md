...
On `status: "completed"` (auth kinds other than `none`): host merges `userConfigPatch` (if any) into submitted `userConfig`, validates merged result against `userConfigSchema`, encrypts creds, then **either inserts a new `service_connections` row (first connect) or rebinds the result onto the user's existing connection for that plugin (reconnect — see _Reconnect rebind_ below)**, auto-promotes to default if first instance, returns connection to frontend. **Empty-creds rows rejected**: if validated credentials payload for plugin with `credentialsSchema` missing required fields | resolves to empty object → create refused with typed error. ⊥ "parked" connections. Exception: `auth.kind: "none"` user-scoped plugins persist with `credentials: {}` by design — `writeConnection({ allowEmptyCredentials: true })`.

Creds & device codes ⊥ logged. `pending_auth` rows have 15-min TTL with nightly sweep.

**Atomic nonce consumption.** Completion paths (`oauth_redirect`, `oauth_device`) consume the `pending_auth` row via `DELETE ... RETURNING` keyed on `(nonce, userId)` BEFORE persisting the `service_connections` row. Only the caller whose DELETE returns a row proceeds to persist (`persistConnectionFromAuth`); concurrent completions for the same nonce see zero returned rows and surface a typed error (`oauth.concurrent_completion` on `completeRedirectAuth`, `{ status: "error", message: "auth nonce already consumed by a concurrent request" }` on `pollDeviceAuth`). Prevents the TOCTOU window where two callbacks racing on the same nonce both pass `loadPendingAuth` and write duplicate connection rows. Trade-off: if the write throws after the DELETE succeeds, the nonce is gone and the user restarts OAuth — accepted, duplicate rows are the worse failure mode.

**Reconnect rebind.** After consuming the nonce, `persistConnectionFromAuth` decides insert-vs-update from the manifest's `poolable` flag. For a non-poolable plugin (one connection per user — e.g. Trakt, Plex) that already has a connection, re-running the OAuth ceremony is a _reconnect_: the fresh credentials are rebound onto that existing row (`reconnectConnection`) — re-encrypted creds, `userConfigPatch` re-applied, `status` → `connected`, `errorMessage` / stale `tokenExpiresAt` cleared — preserving its id, `displayName`, `isDefault`, and `enabled` so primary-provider references and the default flag survive. Poolable plugins, and the no-existing-row case, fall through to a plain insert. This is why the catalog can keep "Connect" disabled for an already-connected non-poolable plugin while the card's `Reconnect` button still re-authorises in place. (Form-auth reconnect uses a different path — `updateUserConfig` re-runs `startAuth` to re-verify, then resets `status` → `connected` — since credentials are re-entered through the edit modal, not an OAuth ceremony.)
...
- **One-shot bucket** — `verify-config`, `:id/test`, `oauth/redirect/start`, `oauth/redirect/complete`, `oauth/device/start`. Capacity 20, refill 20/min. Debit runs _before_ payload validation so malformed probe-spam still consumes tokens.
...
Both return `429` with a `Retry-After` header on throttle.
