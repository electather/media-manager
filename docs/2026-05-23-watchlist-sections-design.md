...
**Status:** design (rev 13)
**Date:** 2026-05-23 (rev 1–6: 2026-05-23; rev 7: 2026-05-25; rev 8: 2026-05-25; rev 9: 2026-05-26; rev 10: 2026-06-01; rev 11: 2026-06-16; rev 12: 2026-06-16; rev 13: 2026-06-17)
...
- **rev 13 (2026-06-17)** — Added a deterministic secondary tie-break to the `sort=alpha` comparator (§S, `sources/items.ts`; issue #680). The rev 12 pin (`localeCompare(_, "en", { sensitivity: "accent" })`) is case-insensitive, so case-variant titles (`elite` / `Elite` / `ELITE`) all collide at `0` and were left to JS sort stability, which is input-order-dependent and diverges across environments. The comparator now appends a secondary full-collation pass — `localeCompare(_, "en", { sensitivity: "accent" }) || localeCompare(_, "en")` — that fires only on a primary tie and orders the colliding titles by full `"en"` collation (e.g. `elite` < `ELITE`), making the order reproducible. This refines the rev 12 pin: the primary key is unchanged; the addition is a deterministic tie-break, not a change to the primary collation semantics.
