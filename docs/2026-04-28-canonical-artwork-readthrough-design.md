...
They are not free, though: the canonical row is shared across all users, so N viewers of the same hot title in the same instant would each fire a redundant COALESCE UPDATE against the one row, scaling WAL traffic with concurrent viewers. To bound that, the artwork service keeps a process-wide write-back dedup window (`WRITE_BACK_DEDUP_MS`, currently 60s): the first fulfilled dispatch for a canonical key claims the window and patches; later dispatches for the same key inside the window skip the patch. A claim carries the timestamp it recorded as a generation token. A failed patch releases the claim only when that token still owns the key, so the next read can retry — best-effort write-back is preserved. The guard matters because a patch can outlive its own window and reject after a newer dispatch has re-claimed the key: an unconditional release would evict that fresher claim and let a redundant patch fire against an already-claimed row. The token is unique enough to distinguish a stale claim from the current owner because a fresh claim only happens once the previous window has fully lapsed. The window is purely a write-amplification guard; correctness still rests on COALESCE, so no DB lock is needed.
