...
Backed by `GET /api/connections/available`. The server-side filter is **already in place** — `listAvailablePlugins` in `packages/server/src/connections/service.ts` already skips any plugin whose `userScopedCapabilities` is empty, which is equivalent to filtering out pure-global plugins. No server change is needed for this affordance; the frontend refactor depends on the existing behaviour. A hypothetical pure-global build never appears here.
...
- `GET  /api/connections/available` — already returns `userScopedCapabilities`, `globalScopedCapabilities`, `adminSharedAvailable`, `poolable`, `userConfigSchema`, `credentialsSchema`; `hasSharedConfig` is already removed; pure-global plugins are already filtered out (the existing `userScopedCapabilities.length === 0` check in `listAvailablePlugins`). One field rename in this PR: `auth` → `authKind` so the endpoint and `/connections/`'s embedded plugin share the `PluginSummary` shape.
