...
| TASK-002 | Create `apps/client/src/shared/components/virtualized/virtual-window-list.tsx` exporting `VirtualWindowList<T>` per spec §API. Props: `items`, `getKey`, `estimateSize`, `renderItem`, `overscan?` (d=4), `header?`, `footer?`, `className?`. Uses `useWindowVirtualizer` w/ `scrollMargin` state synced from `parentRef.current.getBoundingClientRect().top + window.scrollY` via `ResizeObserver(document.body)` (via shared `useScrollMargin` hook). Renders absolutely-positioned children with `ref={v.measureElement}` and `transform: translateY(vi.start - scrollMargin)`. Also pass `getItemKey: i => getKey(items[i], i)` to the virtualizer so the row-measurement cache is keyed by item identity, not index. | ✅ | 2026-05-21 |
...
| TASK-006 | Create `apps/client/src/shared/components/virtualized/__tests__/use-grid-columns.test.ts`: assert col formula at widths {200, 375, 376, 768, 1024}, gap=16, minCol=180 → expected {1,1,2,4,5}. Pivots picked at actual boundaries of `floor((width + gap) / (minCol + gap))`. Mock `ResizeObserver` via `vp test` setup. | ✅ | 2026-05-21 |
...
| TASK-013 | Fire `onRangeChange({ startIndex, endIndex })` from `useEffect` deps on the primitive `startIndex` / `endIndex` derived from `virtualItems` (`virtualItems[0]?.index ?? -1`, last index similarly). Depping on `virtualItems` directly causes the effect to fire every render because TanStack Virtual v3 returns a new array reference each render. | ✅ | 2026-05-21 |
...
| TASK-028 | In filtered mode (`filterActive`): render `<WatchlistFilteredGrid>` directly. Inner `VirtualGrid` provides the virtualization, so no outer `VirtualWindowList` is needed (wrapping a single node buys nothing). Decided. | ✅ | 2026-05-21 |
