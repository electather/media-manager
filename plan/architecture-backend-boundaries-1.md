...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-042 | Retrofit `apps/server/src/preferences/`. Apply canonical layout (PAT-001). Extract `repo/` from existing data-access code (`repo/feedback.ts` + `repo/storage.ts`). Move private helpers under `internal/`. Add `service.ts` with `getPreferencesService()` factory. Author `events.ts` as `PREFERENCES_EVENTS = {} as const` — Phase 3a confirmed preferences is consumer-only; `preferences/jobs/*` job-ID strings travel internally through the service, they are not cross-module events. Wire `preferences.registerJobs()` into `apps/server/src/index.ts` (default `{ scheduled: true }`) and `apps/server/src/worker.ts` (`{ scheduled: false }` — the carve-out skips the daily-rebuild croner registration while keeping the triggerable manual-rebuild + coalesced incremental-update wired for HTTP/MCP triggers). Enable boundaries test coverage for `preferences/`. Add changeset (`@ent-mcp/server: minor`). | ✅ | 2026-05-17 |
