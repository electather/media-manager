...
- **RISK-005**: `MediaService.getMatchingServers` and `getLibraryIndex` memo identity is intentionally deadline-agnostic (first caller's deadline governs the shared probe). Safe today — every `MediaService` instance is scoped to one HTTP request or one warm-job row via `buildContext`. Latent risk: if a `RowContext` or `MediaService` is ever reused across calls with differing deadlines, the tighter deadline silently inherits the looser one. Mitigation deferred: documented inline on both memo caches (`apps/server/src/media/service/index.ts`). Follow-up if reuse emerges — make `MediaService` constructor private, expose `MediaService.forRequest(userId, deadlineMs)` factory so the per-call instance scoping is enforced structurally instead of by convention.
